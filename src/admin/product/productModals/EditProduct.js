...
function EditProduct({selected, handleClose}) {
    ...
    //인풋값
    const [category, setCategory] = useState(selected.categoryId);
    const [name, setName] = useState(selected.item.name);
    const [description, setDescription] = useState(selected.item.description);
    const [price, setPrice] = useState(selected.item.price);
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Select size="lg" onChange={(e) => categoryChange(Number(e.target.value))} isInvalid={invalidCategory}>
                                {mojongs.map((mojong, index) => (
                                    <option key={index} value={index} selected={mojong.id === selected.categoryId}>{mojong.name}</option>
                                ))}
                            </Form.Select>
