...
function DeleteProduct(props) {
    ...
    const deleteProduct = () => {
        console.log('모종 삭제')
        ServerApi('delete', '/item', { 'id': mojong.id }, token, removeToken, updateToken)
            .then(response => {
                //성공
                console.log("성공")
                handleClose();
        ...
        <Modal show={show === 'delete'} onHide={handleClose}>
            <ModalHeader className='border-bottom border-success-subtle border-2'>
                <ModalTitle>
                    ...
                    <span className='ms-2 text-danger fw-semibold'>상품 삭제</span>
                </ModalTitle>
            </ModalHeader>
            ...
            <div className='d-flex gap-1 p-2 border-top'>
                ...
                <Button variant='danger' className='w-100' onClick={() => deleteProduct()}>삭제하기</Button>
            </div>
        </Modal>
    )
}
